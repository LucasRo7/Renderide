...
//!
//! Functionally identical to [`pbsdualsided`](super::pbsdualsided) at the WGSL level — Unity's
//! `alpha`/`Queue=Transparent` directives only change default render queue and blend factors, which
//! this renderer drives from the host-set `_SrcBlend`/`_DstBlend`/`_ZWrite` material properties.
...
/// Material uniforms for `PBSDualSidedTransparent`. Field names match the Unity property block;
/// keywords (`_NORMALMAP`, `VCOLOR_*`, …) are encoded as `f32` and read with `kw_enabled` (>0.5).
struct PbsDualSidedTransparentMaterial {
    ...
    _Metallic: f32,
    /// Alpha-clip threshold (`AlphaClip`); applied only when `_ALPHACLIP` is enabled.
    _AlphaClip: f32,
    ...
    VCOLOR_EMIT: f32,
    /// Keyword: scale metallic/smoothness by vertex color luminance/alpha.
    VCOLOR_METALLIC: f32,
}
...
/// Resolve the [`SurfaceData`] for a fragment, mirroring Unity's `surf` for `PBSDualSidedTransparent`.
fn sample_surface(
    uv0: vec2<f32>,
    world_n: vec3<f32>,
    front_facing: bool,
...
/// Iterate the cluster's lights and accumulate Cook–Torrance radiance, gated by directional/local flags.
