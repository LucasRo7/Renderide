...
struct FiltersInvertPerObjectMaterial {
    _Lerp: f32,
    _GrabPass: f32,
    _pad0: vec2<f32>,
}

@group(1) @binding(0) var<uniform> mat: FiltersInvertPerObjectMaterial;
...
fn fs_main(
    ...
    let filtered = mix(c.rgb, 1.0 - c.rgb, clamp(mat._Lerp, 0.0, 1.0));
