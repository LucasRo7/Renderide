//! WGSL shader sources for skinned mesh pipelines (bone-weighted vertex transform, MRT variant).

/// Skinned mesh shader: blendshape offsets → bone-weighted transform → world-normal coloring.
///
/// Bind group 0: uniform (slot 0, dynamic offset = per-draw skinned uniforms), storage (slot 1 = blendshape offsets).
...
/// Skinned MRT shader: same bone skinning as [`SKINNED_SHADER_SRC`] with three-target G-buffer output.
///
/// Bind group 1: `MrtGbufferFrame` uniform for camera-relative position encoding.
