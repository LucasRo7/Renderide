...
//!
//! The live non-skinned forward shader now comes from `RENDERIDESHADERS/pbr/pbs_metallic.wgsl`
//! via `build.rs`. MRT and skinned variants remain inline until they are migrated to the same
//! source-of-truth folder.

/// PBR shader: Renderide PBS metallic forward shader built from `RENDERIDESHADERS`.
pub(crate) const PBR_SHADER_SRC: &str = include_str!(concat!(env!("OUT_DIR"), "/pbs_metallic.wgsl"));
